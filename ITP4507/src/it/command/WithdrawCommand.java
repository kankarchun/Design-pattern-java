...
public class WithdrawCommand implements Command {
    ...
    private Security s;
    private int withdraw,total;
    private Stack undo, redo;
    private Stack undoList, redoList;
    private String error;

    public WithdrawCommand(Stack undo, Stack redo, Security s, int withdraw,String error) {
        this.undo = undo;
        this.redo = redo;
        this.s = s;
    ...
    public void execute() {
        if (error != null) {
            ...
            total=s.getQuantity()-withdraw;
            //set the securit new name
            s.setQuantity(total);
            //push memento in undo list stack
            undoList.push(m);

            System.out.println("Withdrew " + withdraw + " to " + s.getCode() + ". Current quantity is " + s.getQuantity());
        }
    }
    ...
    public String showCmd() { //string of show list of undo redo stack
        return "Withdraw " + withdraw + " " + s.getCode();
    }
}
